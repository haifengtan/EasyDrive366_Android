...
public class UesrProfileActivity extends BaseHttpActivity {
	private String data;
	...
	protected void onCreate(Bundle savedInstanceState){
		...
		String url = String.format("upload/init_user_docpic?userid=%d", AppSettings.userid);
		beginHttp();
	...
	private void onCellImageClick(Object obj){
		try{
			JSONObject json = (JSONObject)obj;
			String url = String.format("upload/get_user_docpic?userid=%d&typeid=%d", AppSettings.userid,json.getInt("id"));
			beginHttp();
			new HttpExecuteGetTask(){
	...
	private void sendImageToServer(String filename) {
		...
		try {
			String actionUrl = String.format(
					"%supload/upload_user_docpic?userid=%d&typeid=%d",
					AppSettings.ServerUrl, AppSettings.userid,_typeid);
